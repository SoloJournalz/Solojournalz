      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-[1fr_150px] gap-3">
          <button
            ...
            className="rounded-xl bg-[var(--accent)] px-4 py-3 text-left text-sm font-semibold text-white shadow-[0_8px_20px_rgba(110,17,17,0.18)] transition disabled:cursor-not-allowed disabled:opacity-40"
          >
            Delete Trade
          </button>
          ...
          <select
            ...
            className="rounded-xl border border-[#d8d5cf] bg-[#efeee9] px-4 py-3 text-sm font-semibold text-[var(--text-secondary)] outline-none focus:border-[var(--accent)]"
          >
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {trades.map((trade, index) => (
                ...
                >
                  #{index + 1} · {trade.strategy || "No Strategy"} ·{" "}
                  {trade.pair}

                  <span className="block pt-1 text-xs opacity-70">
                    {trade.environment || "N/A"} · {trade.result || "N/A"} ·{" "}
                    {formatPnl(trade.pnl)} · {trade.trade_date} ·{" "}
                    {trade.id.slice(0, 8)}
                  </span>
                </button>
              ))}
